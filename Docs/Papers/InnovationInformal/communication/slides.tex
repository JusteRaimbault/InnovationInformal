...
\textit{Special session ALife and Society}\\
July 19th 2022
}
...
\sframe{Sustainable development goals}{
...
\includegraphics[width=0.38\linewidth]{figures/alifemainpage.jpg}
\includegraphics[width=0.58\linewidth]{figures/sdgs_poster_936_en.png}

\bigskip

\textit{Improvement of real lives? (DOLCE vita)}

\bigskip

$\rightarrow$ challenges of sustainable developments goals \cite{nations2016sustainable}: necessary transitions of multiple interconnected socio-technical systems

}

\sframe{SDGs and innovation}{

$\rightarrow$ \textbf{Innovation} central to evolution and artificial life

\smallskip

$\rightarrow$ Central element at the intersection of several SDGs: 9 (innovation and infrastructure), 8 (economy), 11 (cities), 13 (climate)

\smallskip

$\rightarrow$ Emergence of trade-offs between SDGs in systems of cities: example of innovation and emissions \cite{raimbault2022trade}

\bigskip

\centering

\includegraphics[height=0.5\textheight]{figures/jum_paper.png}
...
Literature on \textbf{firm clusters} in economic geography and regional science:

\medskip

\begin{itemize}
	\item Positive impact on innovation capabilities \cite{bittencourt2019cluster}
	\item Intensity of social interaction and exchange of tacit knowledge \cite{gnyawali2013complementary} \cite{arikan2009interfirm}
	\item Transfer of competences through employees \cite{almeida1999localization}
\end{itemize}
...
Innovation emerging from the bottom-up, making agent-based models relevant:

\medskip

\begin{itemize}
	\item spatial diffusion of innovations \cite{kiesling2012agent}
	\item collective decision making and creativity \cite{sayama2015studying}
	\item innovation niches \cite{lopolito2013emerging}, patenting \cite{dosi2021patents}, evolution of technologies \cite{chen2006functional}
	\item role of space: multiple scales \cite{vermeulen2018role}
	\item Properties and effects of firm clusters \cite{fioretti2005agent}
\end{itemize}
...
$\rightarrow$ At the crossroads of these two literature streams, role of informal knowledge flows at the microscopic level within firm clusters?

\medskip

$\rightarrow$ Practical implications for urban and regional planning, and the (non-)implementations of clusters
...
\begin{enumerate}
	\item Simple ABM simulating innovation within firms and informal knowledge flows between firms, similar to biogeography evolutionary algorithms
	\item Instanciation on a specific synthetic fitness landscape
	\item Systematic exploration and sensitivity analysis
\end{enumerate}
...
\begin{itemize}
	\item Firms of size $S_k$ as set of employees $e_{ki}$ with $1 \leq i \leq S_k$, represented by their ideas (genotype) $e_{ki} = (x^{(ki)}_j) (t) \in \mathbb{R}^G$.
	\item Geographical crossover between firms captures informal knowledge flows.
	\item Intra-firm evolution with selection of ideas driven by a fitness landscape.
	\item Companies have a ``current product'' $p_k (t) = (p_{kj})(t)\in \mathbb{R}^G$ with the corresponding fitness value $y_k (t)$ (social utility or company turnover).
\end{itemize}
...
%    \item Ideas are exchanged within firms, corresponding to the core of the genetic algorithm capturing innovation. Each employee has a fixed probability $p_C$ of realising a crossover with another one. Following a random draw, if this is realised, one other employee is selected at random, and the current employee copies a fixed share $s_C$ of the other genome (obtained in practice with a random draw of probability $s_C$ for each gene). The update is done synchronously so that no propagation can occur within one time step. The exchange is not symmetric (reflecting the asymmetry of idea exchanges in real life), but each employee gets to renew its ideas. Genomes are then mutated with a probability $p_M$ (at the gene level for all employees), and mutations correspond to a uniform random increment $m \in \left[ -x_M/2 ; x_M/2 \right]$ where $x_M$ is a parameter giving the amplitude of the mutation.
	\item Evolution of ideas within firms: probability of crossover for each employee $p_C$ (copy of a share $s_C$ of genome of one other random employee); mutations with probability $p_M$ at the gene level (uniform variation $m \in \left[ -x_M/2 ; x_M/2 \right]$).
%    \item New ideas are tried by employees, in other words the fitness function $y$ is evaluated for each genome $y_{ki} = y(e_{ki})$. Within each company, the next product is selected as the one maximising the fitness: $p_k (t) = \textrm{argmax}_i y_{ki}$ and the corresponding fitness value is taken as $y_k (t)$. At this stage, the analogy with the genetic algorithm is slightly modified to reflect actual research processes within firms: a fixed share of employees $s_P$ of the firm is randomly chosen to work on the product during the next cycle, and thus update their genome as the product genome $e_{ik} = p_k$. This leads to a loss of diversity which may be detrimental to a genetic algorithm with the sole aim of optimisation, but this is not the purpose of our model which is to capture actual innovation processes in a stylised way.
	\item Implementation of new ideas: evaluation of the fitness function $y_{ki} = y(e_{ki})$, selection of the best product $p_k (t) = \textrm{argmax}_i y_{ki}$; fixed share of employees $s_P$ are selected to work on it in the next cycle and thus update their genome $e_{ik} = p_k$.
...
%    which corresponds to a classic spatial interaction model \citep{wilson1975some}. Two interacting employees will act as for the internal crossover: the first agent copies a random part of the genome using the $s_C$ parameter.
	\item Informal knowledge between firms: probability of interacting for two employees of different firms given by a spatial interaction model
	\[
    p_{ij} = p_E \cdot \exp{\left( - d(k_i,k_j) / d_E \right)}
    \]
\end{enumerate}
...
\begin{enumerate}
	\item Average fitness between firms $\bar{f} (t)$
	\bigskip
	\item Inequality between firms: entropy $\mathcal{E}_f$ of fitnesses
	\bigskip
	\item Diversity of products
	\[
	d(t) = \frac{1}{2 \cdot N_f \cdot (N_f - 1)} \sum_{k \neq l} \left(1 - \frac{p_k (t)\cdot p_l (t)}{\left||p_k (t)\right|| \cdot \left||p_l (t)\right||}\right)
	\]
\end{enumerate}
...
\begin{itemize}
%A certain number of parameters can be parametrised to match realistic configurations. The size of firms can in practice be approximated by a power law for the largest sizes of the distribution \citep{growiec2008size}. Therefore, we distribute $S_k$ following a rank-size law $S_k = S_0 \cdot k^{-\alpha_S}$ where indices are in decreasing size order, $S_0$ is the size of the largest firm and $\alpha_S$ the level of hierarchy.
	\item Firm size follows a rank-size law $S_k = S_0 \cdot k^{-\alpha_S}$, with $S_0 = 100$ and $N_f = 10$ (medium-size cluster).
%Locations of firms are taken randomly in $\left[0 ; 100 \right]^2$, such that the order of magnitude for the $d_E$ is in the same interval (corresponding to realistic sizes of urban regions where clusters are generally located).
	\item Random location of firms $\left[0 ; 100 \right]^2$ (one cluster simulated).
%Initial employee genomes are initialised randomly in $[-10 ; 10]$ and the initial product and corresponding fitness are chosen randomly among employees.
	\item Initial random genomes $[-10 ; 10]$, random initial product.
...
%where $m_{ij}$ is a random uniform static matrix of size $G \times G$ and with coefficient in $[0;1]$, capturing the random fitness landscape used by  \cite{ma2005agent}, and the rest is the classic Rastrigin function.
	\item Fitness landscape: random fitness landscape \cite{ma2005agent} combined with a Rastrigin function
\[
y(\vec{x}) = - \sum_{i,j} m_{ij} \left[x_i^2 - 10 \cos\left(2 \pi x_i\right) \right]
\]
%We furthermore fix a certain number of parameters which can reasonably correspond to real world values. We take medium-sized companies by taking $S_0 = 100$ and a number of firms $N_f = 10$, corresponding to a medium-sized cluster, such as in the case of several start-ups working in digital services. We fix the genome size at $G = 10$ to avoid exploring too large dimensional spaces. We run the model with $t_f = 100$, corresponding to a magnitude of 10 years if one time steps is roughly one month. The rest of the parameters are left free and will be explored in the numerical experiments.
	\item Genome size $G = 10$; final time $t_f = 100$ ($\sim$10y).
\end{itemize}
...
$\rightarrow$ Model implemented in scala.

\medskip

$\rightarrow$ Explored and validated using the OpenMOLE software \cite{reuillon2013openmole}

\medskip	
	
	\begin{center}
	\includegraphics[width=0.1\textwidth]{figures/iconOM.png}
    \includegraphics[width=0.45\textwidth]{figures/openmole.png}
	\end{center}
...
\begin{itemize}
	\item firm size hierarchy $\alpha_S \in [0.1 ; 2.0]$
	\item crossover probability $p_C \in [0 ; 1]$
	\item crossover share $s_C \in [0 ; 1]$
	\item mutation probability $p_M \in [0 ; 1]$
	\item mutation amplitude $x_M \in [0 ; 2]$
	\item product work share $s_P \in [0 ; 1]$
	\item interaction probability $p_E \in [0 ; 10^{-4}]$
	\item distance decay $d_E \in [1 ; 100]$
\end{itemize}
...
\textbf{Internal model validation:} 100 parameter points (Latin Hypercube Sampling) with 1000 stochastic replications each.

\bigskip

$\rightarrow$ relatively low stochastic variability: all indicators with Sharpe ratios above 4, except $\Delta f$ with a median of 1.48.

\bigskip

$\rightarrow$ distance between average relative to standard deviations is high (median above 2.5).
...
Global sensitivity analysis indices \cite{saltelli2008global}
...
\hline
 & \multicolumn{2}{|c|}{$\alpha_S$} & \multicolumn{2}{|c|}{$p_C$} & \multicolumn{2}{|c|}{$s_C$} & \multicolumn{2}{|c|}{$p_M$}   \\
 & F & T & F & T & F & T & F & T \\
 \hline
$b$ & 0.001 & 0.002 & 0.001 & 0.003 & $9\cdot 10^{-4}$ & 0.002 & 0.41 & 0.75 \\
$\bar{f}$ & 0.02 & 0.07 & $6\cdot 10^{-4}$ & 0.002 & 0.0 & 0.003 & 0.36 & 0.69 \\
$\Delta f$ & $7\cdot 10^{-4}$ & 0.56 & 0.0 & 0.9 & 0.0 & 0.0 & 0.003 & 0.0 \\
$\mathcal{E}_f$ & 0.14 & 0.64 & 0.0 & 0.44 & 0.27 & 0.36 & 0.48  & 0.84 \\
$d$ & 0.007 & 0.13 & 0.001 & 0.04 & 0.01 &  0.1 &  0.45  & 0.7 \\\hline
\end{tabular}
...
\hline
& \multicolumn{2}{|c|}{$x_M$} & \multicolumn{2}{|c|}{$s_P$} & \multicolumn{2}{|c|}{$p_E$} & \multicolumn{2}{|c|}{$d_E$} & \multicolumn{2}{|c|}{seed} \\
 & F & T & F & T & F & T & F & T & F & T \\
 \hline
$b$ & 0.17 & 0.52 & 0.03 & 0.13 & $5\cdot 10^{-4}$ & 0.002 & $9\cdot 10^{-4}$ & 0.002 & 0.003 & 0.007\\
$\bar{f}$ & 0.21 & 0.55 & 0.02 & 0.008 & 0.0 & 0.004 & $4\cdot 10^{-4}$ & 0.004 & $8\cdot 10^{-4}$ & 0.007\\
$\Delta f$ & 0.0 & 0.24 & 0.0 & 0.48 & 0.0 & 0.18 & 0.0 & 0.17 & 0.0 & 0.0 \\
$\mathcal{E}_f$ & 0.014 & 0.35 & 0.23 & 0.41 & 0.16 & 0.39 & 0.0 & 0.40 & 0.05 & 0.46 \\
$d$ & 0.21 & 0.42  & 0.0  & 0.1 & 0.003 & 0.09 & 0.006 &  0.09 & 0.006 & 0.05 \\\hline
\end{tabular}
...
$\rightarrow$ stronger effect of mutation $p_M$ (fixed in the following); no effect of spatial interaction on diversity but on inequality.
...
\begin{center}
\includegraphics[width=0.8\linewidth]{../InnovationInformal-paper/figures/averageFitness-distanceDecay_color-interactionProba_facet-crossOverShare-firmSizeScaling_crossOverProba0.5.png}
\end{center}

\medskip

Average fitness behavior: strong but plateauing effect of informal knowledge; unequal firm sizes is less optimal.
...
\begin{center}
	\includegraphics[width=0.8\linewidth]{../InnovationInformal-paper/figures/diversity-distanceDecay_color-interactionProba_facet-crossOverShare-firmSizeScaling_crossOverProba0.5.png}
\end{center}

\medskip

Diversity behavior: higher fitnesses correspond to lower diversity; qualitative regime change for low $\alpha_S$ when $p_E$ increase; slight effect of $s_C$.
...
\begin{center}
\includegraphics[width=0.6\linewidth]{../InnovationInformal-paper/figures/paretoDiversity-Fitness_colorinteractionProba.png}
\end{center}
...
Pareto front between utility and the opposite of diversity obtained with a NSGA2 algorithm; U-shape behavior of interactions along the front; compromise points: long range interactions with a low probability, equal firm sizes, small product share (importance of intra-company diversity).
...
\textbf{Main findings:} (i) important role of informal knowledge exchanges; (ii) hierarchy between firms is less optimal; (iii) trade-off between innovation fitness and diversity (compromise points correspond to equal-size regional cluster).

\bigskip

\textbf{Link with empirics:} role of spatial niche for diversity \cite{dionne2019diversity}; role of local exchanges \cite{fitjar2014local}; no observation of ``agglomeration diseconomies'' on fitness but on diversity \cite{folta2006geographic}.
...
\begin{itemize}
	\item More stylised facts to reproduce \cite{gnyawali2013complementary}
	\item Multi-scale model with multiple clusters: coupling with urban evolution model \cite{raimbault2020model} 
	\item Role of telework \cite{bergeaud4015066telework}
	\item Parametrisation on real-world data, including patent data
\end{itemize}
...
$\rightarrow$ Simple ABM of a firm cluster with informal knowledge flows.

\bigskip

$\rightarrow$ Optimal compromise systems found to be regional equal firm systems.

\bigskip

$\rightarrow$ Basis for more advanced models in link with policies, other scales and dimensions of innovation and SDGs.
...
\textbf{Model code open source at }
...
\bibliography{../InnovationInformal-paper/biblio,biblio}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
